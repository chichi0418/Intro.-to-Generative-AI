...
function renderUserContent(content: string | ContentPart[]) {
  if (typeof content === 'string') {
    return <span className="whitespace-pre-wrap break-words">{content}</span>;
  }
  const images = content.filter(p => p.type === 'image');
  const texts = content.filter(p => p.type === 'text').map(p => (p as { type: 'text'; text: string }).text).join('');
  return (
    <>
      {images.length > 0 && (
          ...
          {images.map((p, i) => {
            ...
            return (
              <img
                ...
                style={{ maxWidth: 240, maxHeight: 200, objectFit: 'cover', border: '1px solid var(--line)' }}
              />
            );
          })}
        </div>
      )}
      {texts && <span className="whitespace-pre-wrap break-words">{texts}</span>}
    </>
  );
}
...
export function MessageBubble({ message, onEdit, onRegenerate }: Props) {
  ...
  const [editText, setEditText] = useState(getTextContent(message.content));
  ...
  function handleEditKeyDown(e: React.KeyboardEvent<HTMLTextAreaElement>) {
    ...
    if (e.key === 'Escape') {
      setEditing(false);
      setEditText(getTextContent(message.content));
    }
  }
          ...
          {editing ? (
            <div className="w-full" style={{ minWidth: 280 }}>
              ...
              <div className="flex justify-end gap-2 mt-2">
                <button
                  onClick={() => { setEditing(false); setEditText(getTextContent(message.content)); }}
            ...
            <>
              ...
              {onEdit && (
                <button
                  onClick={() => { setEditText(getTextContent(message.content)); setEditing(true); }}
  ...
  return (
    <div className={`flex justify-start mb-5 px-4 gap-3 group ${textContent || (message.toolSteps?.length ?? 0) > 0 ? 'items-start' : 'items-center'}`}>
      <div
            ...
            <ReactMarkdown
              ...
              components={{
                ...
                code: ({ inline, children, ...props }: any) =>
                  inline ? (
                    <code className="px-1.5 py-0.5 rounded text-xs font-mono" style={{ background: 'var(--bg-soft-hover)', color: 'var(--code-text)', border: '1px solid var(--line)' }} {...props}>
                      {children}
                    </code>
                  ) : (
