const { getDbPool, handleDbError } = require('./db');
...
async function consumeServerKeyQuota(ip) {
  try {
    ...
    handleDbError(err, 'consumeServerKeyQuota');
...
async function getServerKeyUsageSnapshot() {
  try {
    ...
    handleDbError(err, 'getServerKeyUsageSnapshot');
...
async function resetServerKeyUsageByIp(ip) {
  try {
    ...
    handleDbError(err, 'resetServerKeyUsageByIp');
